# CLAUDE.md — 德國知識小種子 Das deutsche Wissen

本檔為 Claude Code 在此 repo 工作時的長期記憶與品牌守則。每個 session 開啟時都會自動讀取。

---

## 🧭 True North —— 全流程自動化（用戶 2026/07 定調）

**本專案的主旨：一切以自動化為目標，讓每週產出到發布完全不需要用戶手動操作。**

- 內容產出自動化：圖卡（Pillow）、`js/data.js`、靜態頁 build、FB/IG 雙版貼文——一次全部備齊。
- **社群發布自動化（2026/07 起）**：FB 與 IG 貼文一律走 Meta Graph API 排程
  （`scripts/social-post.mjs`，比照 oasis 的 `fb-post.mjs` 架構），**不再手動貼文**。
  FB 用原生排程、IG 用本機 launchd 到點自動發。詳見 `scripts/README_social.md`。
- 遇到還在手動做的重複步驟，主動提出並實作自動化方案。
- **內容飛輪（用戶 2026/07 定調，2026/08 加上節制原則）**：做圖卡與 FB/IG 貼文時帶入網站既有文章——
  加「📖 延伸閱讀：這篇你可能會喜歡」導流舊文，放大既有內容的效果；
  FB/IG 發文內容要與網站已公開文章**緊緊相扣**（同源一致、互相導流）。
  ⚠️ **但要適量，寧缺勿濫（用戶 2026/08/14 要求）**：「推太多自己的文章反而像是為了推而推。」
  判準是**關聯必須直接**——同主題的自然延伸才放（例：早鳥退休金 → 退休金帳戶查詢），
  轉一手的聯想就是「為推而推」，不要放（例：油價上漲 → 交通月票，已被用戶要求拿掉）。
  **一篇週報頂多一兩則延伸閱讀**；找不到真正貼題的舊文就不放，不要硬湊。
- **舊文查核**：帶入或引用舊文前，先查核內容是否仍屬實；需要就更新，並在 `js/data.js`
  該文加 `updated: 'YYYY-MM-DD'`（build.js 會顯示「發布＋更新」雙時間標記、寫入
  sitemap lastmod 與 article:modified_time）。網站文章盡可能都打上時間標記。
- **圖卡視覺升級（用戶 2026/07 要求；W29 起為固定流程）**：不要純文字牆——**每張週報圖卡
  都必須含一個「Seedling Flat」扁平風精緻插畫**（Pillow 畫幾何圖形：粗深藍描邊 #1F2937、
  品牌配色填充、2x 超取樣抗鋸齒）。產圖一律用 `scripts/cardgen/make_weekly_cards.py`
  （版型＋插畫元件庫），設計哲學見 `scripts/cardgen/design_philosophy_seedling_flat.md`。
  ⚠️ **插畫嚴禁蓋到或貼近文字（用戶 W29 明確要求）**：僅允許出現在右上安全區
  （x≈762–922、y≈326–448），與副標保持 ≥18px、與 bullets 起點保持間隔，適度留白。
  另參考 `assets/images/2026/tax/Steuerfrist2026_圖卡_報稅期限提醒.png` 的鬧鐘＋雙色膠囊標籤。
- 自動化不取代審閱：內容仍先給用戶過目；push／開 PR／合併／正式排程發文前仍要用戶同意
  （見下方工作流程的 ⚠️ 規則）。同意後的「執行」要全自動、一氣呵成。

---

## 品牌名稱（最重要）

中文：**德國知識小種子**
德文：**Das deutsche Wissen**

### 德文文法說明（避免再犯）

- `das` = 中性定冠詞（Nominativ）
- 後接形容詞時必須做「弱變化」，字尾加 **-e** → `deutsche`
- `Wissen`（知識）為中性名詞（das-Wort）
- 完整正確寫法：**`Das deutsche Wissen`**

### 常見錯誤

❌ `Das deutsch Wissen`（缺 -e 字尾）
❌ `Das Deutsche Wissen`（形容詞首字母不大寫；除非品牌刻意風格化，否則一般德文不大寫）
✅ `Das deutsche Wissen`

### 套用範圍

任何包含品牌德文名稱的物件都必須使用正確寫法：
- 圖卡 footer / 浮水印
- 網站 `<title>`、`<meta>`、Open Graph、Twitter Card
- Facebook 粉專名稱與簡介
- Instagram bio
- email signature
- RSS feed `<title>`
- 任何貼文署名 / 引文出處

若在 repo 內看到舊的錯誤寫法 `Das deutsch Wissen`，順手一併更正。

---

## 週報圖卡規範

### 字體（已驗證）

對齊 `css/style.css` 主字體變數 `--font: 'Noto Sans TC', ...`：

- **CJK**：Noto Sans CJK TC
  - Regular：`/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc` index=3
  - Bold：`/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc` index=3
- **Latin / 數字 / 符號**：Noto Sans（W24 起；apt 套件 `fonts-noto-core`）
  - Regular：`/usr/share/fonts/truetype/noto/NotoSans-Regular.ttf`
  - Bold：`/usr/share/fonts/truetype/noto/NotoSans-Bold.ttf`
  - 背景：W12–W23 用 DejaVu Sans，但用戶反映 DejaVu Bold 數字過粗不易讀，
    W24 起改用與 CJK 同家族的 Noto Sans，**之後一律用 Noto Sans、勿用回 DejaVu**

注意：原始任務 spec 文字描述提到的 `DroidSansFallbackFull.ttf` 與環境中的 `wqy-zenhei` 都**不符**既有圖卡視覺。網站本身使用 Noto Sans TC，圖卡應與網站視覺一致 → Noto Sans CJK TC 才是正解。

### 字重層級

- **Bold**：標題、stats 大數字、bullet header、badge、takeaway 標籤
- **Regular**：副標、stats label、bullet 內文、takeaway 內文、footer

### 規格

- 尺寸：1080×1080 PNG
- 背景：`#F8F6F2`（暖奶油色）
- 卡片：`#FFFFFF`，邊框 `#E8E4DE`
- 圓角矩形 + 彩色圓點 bullet
- 頂部色條 + 分類 badge
- **右上角品牌 lockup（用戶 2026/07 定稿）**：一律貼「手寫字＋小6圈圈底線＋🌱」lockup，
  `make_weekly_cards.paste_wordmark()` 會自動貼上——**勿再用 Noto 印刷體畫右上角品牌字**。
  原稿是用戶以 Claude Design 畫的 `assets/images/Social Media/brand_手寫風.png`（勿改動），
  經 `scripts/cardgen/prep_brand_lockup.py` 去背輸出 `scripts/cardgen/brand_lockup.png` 給產卡用；
  原稿更新後重跑 prep 腳本＋重產圖卡即可（去背用背景色距離法，勿用灰階法——會毀掉彩色 🌱）。
  另：badge 等框內文字用 `draw_mixed_vcentered()` 以實際字面 bbox 垂直置中，勿手動加 offset
- 主題色：綠 `#2E8B57`、紅 `#C0392B`、橙 `#D4740E`、藍 `#2563EB`、紫 `#7C3AED`、青 `#0D9488`

### 精緻插畫（Seedling Flat，W29 起每張必備）

- **產圖工具**：`python3 scripts/cardgen/make_weekly_cards.py <輸出資料夾>`——
  改檔尾 `WEEK`、`DATE_RANGE`、`CARDS` 區塊即可產出整週圖卡（版型與插畫都內建）。
- **版型**：stats 兩格（自 W29 起，第三格空間讓給插畫）；插畫置於右上安全區、
  帶主題色 10% 淡色圓底。
- **插畫元件庫**（`ILLUS`，持續擴充）：`podium` 麥克風講台（記者會／官方聲明）、
  `flags` 德法雙旗（外交，可仿造改其他國旗）、`coins` 金幣＋上升箭頭（財政／物價）、
  `idcard` 居留卡＋晶片＋指紋（居留／簽證）、`camera` 監視器＋人臉框（監控／治安）、
  `trophy` 獎盃＋足球（賽事）、`piggybank` 小豬撲滿＋金幣（兒童儲蓄／長期資本累積）、
  `shieldeye` 盾牌＋監看之眼（情報機關／國安權限）、`agelimit` 啤酒杯＋年齡徽章（青少年保護）、
  `heathealth` 生命徵象監測＋烈日（高溫致死／健康衝擊）、`harvest` 麥穗＋乾裂土地（乾旱歉收）。
  新主題若無合適元件，**依設計哲學新增一個**，不要湊合。
- **Seedling Flat 鐵則**（詳見 `design_philosophy_seedling_flat.md`）：
  描邊一律 #1F2937（5px @1080）；每卡限「主題色＋金 #F0C24B＋白」三色系；
  分層表達深度（無漸層、無透視）；單一焦點細節；奇數顆韻律圓點；乾淨的同色系陰影。
- **留白鐵則（用戶 W29 要求）**：插畫任何元素（含彩紙、陰影、圓底）不得超出安全區
  x∈[762, 922]、y∈[326, 448]；不得蓋到或貼近任何文字——寧可縮小插畫也要留白。

### 行距與留白（用戶 2026/07 要求，每張卡必守）

- **標題與副標之間行距要留足**：標題行高至少 1.3 倍字級再加空隙，絕不可讓兩行幾乎貼在一起。
- **插畫一律限制在獨立的水平帶內**：所有插畫元素（主圖、徽章、星星、裝飾圓）都不得
  超出該帶，帶的上下緣與相鄰文字要有明顯空隙——插畫不可壓到、貼到任何文字。
- **交付前必須先自行檢視整張輸出圖**（Read 渲染出的 PNG）：確認無元素重疊、無文字
  被裁切或溢出卡片邊界後才交給用戶。
- **新卡必須與既有同系列卡逐項比對結構**（不能只憑印象）：頂部色條高度 44px、
  底邊平直、僅上方兩角圓角（Pillow `rounded_rectangle` 的 `corners=(True,True,False,False)`）、
  卡片米灰邊框 3px、badge 與色條之間留白。目測不可靠——用 Python 對既有卡做
  像素掃描（中央垂直線找色條範圍、左緣水平線找邊框寬）驗證一致才算過關。
  已踩過的雷：色條畫成 76px 厚圓底、直接切到 badge；邊框畫 2px 看起來像沒有框。

### 術語呈現（社群受眾看得懂為準）

- **德國稅卡（Steuerklasse）的羅馬數字要加註中文級數**：如「稅卡 VI＝第 6 級」、
  「III/V（第 3＋5 級）」，不能只丟羅馬數字。
- **金額頻率用自然中文**：寫「每年超過 410 €」，不要寫「410 €／年」這種表格式縮寫。
- **億／萬級金額一律寫「歐元」全字**（用戶 2026/07 要求）：「5,554 億歐元」，
  嚴禁「5,554 億 €」這種中文單位＋符號混排；「歐元」後接中文不留半形空格。
  純數字金額（410 €、25 €）維持 € 符號。標題出現金額也必須帶幣別。
- Faktor（Faktorverfahren）＝配偶間依收入比例分攤預扣稅的「因子法」，**與子女無關**，
  勿寫成「子女因子」。

### 既有圖卡與檔名

- 路徑：`assets/images/2026/weekly/`
- 命名：`W{週數}_圖卡{序號}_{主題}.png`
- 例：`W23_圖卡1_德國隊出征世足.png`

### 📅 日期一律寫「整週」（用戶 2026/08/27 要求）

**週報的日期範圍一律寫該 ISO 週的完整區間（週一–週日），不可以「跑稿當天」為結尾。**
例：第 35 週就寫 `2026/08/24-08/30`，即使稿子是週四（8/27）產出的，也不能寫成 `08/24-08/27`。

四處都要照同一個整週區間：

| 位置 | 寫法 |
|---|---|
| 圖卡底部註腳 | `make_weekly_cards.py` 的 `DATE_RANGE = '2026/08/24-08/30'` |
| `js/data.js` 的 `date:` | 該週**週日**（如 `'2026-08-30'`） |
| `<h2 id="...">` 與 TOC | 錨點與標題都用整週（如 `id="aug24-30"`、「8月24日–30日：…」） |
| FB／IG 貼文標頭 | 「德國週報 W35｜8/24–8/30」 |

理由：週報是「這一週的德國」，註腳寫成半週會讓讀者以為漏了後幾天；
社群排程時間也應排在該區間結束日（或之後），避免貼文比標示的日期範圍還早出去。

### 編輯方針（品質要求）

- **避免「垃圾新聞」**：每張卡需有成因分析 + 專家觀點 + 對讀者的實質意義
- **底部標籤不要每張都寫「對在德台灣人的意義」**（用戶已多次反映很制式、很奇怪）：
  相關性自然寫進內文即可，底部觀察框標籤請依主題分別取名，例如
  影響評估／因應建議／訂票提醒／看球提醒／數據解讀／觀察重點／政策觀察 等；
  最多一張卡用「對在德台灣人的意義」，切勿每張重複同一標題。
- **用專業術語、勿自創口語**：避免「錢的攤牌」「大撒幣」這類口語化標題；
  例：歐盟七年期預算寫「多年期財政框架（MFR / MFF）」，不要寫「錢怎麼分／錢的攤牌」。
- 🚫 **禁用「白拿」**（用戶 2026/09/16 二度糾正）：貼文與圖卡標題不可寫
  「【白拿 15 €】」「又能白拿 X €」這類貪小便宜的口語，與知識型品牌定位衝突。
  改寫成講清楚機制的中性說法：「線上刷卡 2% 現金回饋」「每人最高 15 €」「回饋上限 15 €」。
  同類要避開的還有「免費送」「不拿可惜」「穩賺」。交付前把「白拿」一併列入黑名單 grep。
- **譯名與用詞一律遵循台灣慣例、嚴禁中國譯名與用語**——詳見下方獨立章節
  「🚨 譯名紅線」，那是本專案最容易犯錯也最不能犯錯的一條。
- **提及德國聯邦邦務必附德文**（全名或標準簡寫，擇一）：巴登-符騰堡（Baden-Württemberg／BW）、
  萊茵蘭-普法茲（Rheinland-Pfalz／RLP）、薩爾蘭（Saarland）、黑森（Hessen）、
  北威邦（Nordrhein-Westfalen／NRW）、巴伐利亞（Bayern）等；**勿用「巴符、萊普」這類自創簡稱**。
- **專業記者語氣、直述事實**：是技術故障就寫「系統故障」，**不要用「非罷工／不是…」這種多餘的否定式強調**。
- **不重複老議題**：先比對前幾週（W12 起的 `assets/images/2026/weekly/`）已報過的題目，避免老調重彈
- **冷僻地名 / 人名**：附上中譯與背景脈絡（例：Curaçao → 古拉索／加勒比小國）
- **數據必須有來源**：Facebook 貼文最後附上原始連結

### 🚗 汽車產業：固定追蹤題材（用戶 2026/09 要求）

**汽車是德國最重要的產業，讀者關注度極高——每週跑稿時固定搜尋一次車廠與供應商動向，有題就做。**
不必等到出大事才查；OEM 的新動向本身就有熱度。

- **固定搜尋對象**
  - OEM：Volkswagen（福斯）、Audi、Porsche（保時捷）、BMW、Mercedes-Benz（賓士）、Opel
  - 供應商：Bosch、ZF、Continental、Schaeffler、Mahle、Brose、Aumovio
  - 產業組織與工會：VDA（德國汽車工業協會）、IG Metall、各廠員工代表會（Betriebsrat）
- **優先抓的題型（熱度由高到低）**
  1. **裁員、關廠、廠區檢討**——最高熱度，幾乎一定做（例：W38 福斯十萬個職位）
  2. **虧損、獲利預警、撙節方案**
  3. **罷工與勞資協商**（IG Metall、Betriebsversammlung）
  4. **供應商破產**（近年連環發生，中小廠尤其）
  5. 關稅、中國競爭、電動化轉型進度
  6. 新車與電池／軟體策略（熱度較低，除非牽涉在德就業）
- **寫法要點**
  - **裁員數字務必拆解來源**：德國車廠常把「已議定」與「新增」疊加對外公布，中文報導很容易壓縮成
    「裁員 N 萬人」。要寫清楚是累計還是單次、是上限情境還是已定案，並查有沒有內部人士或工會的不同說法。
  - 德國大規模人力調整多走**自願方案＋自然離職**（Altersteilzeit、Fluktuation、Aufhebungsvertrag），
    不是直接解僱——對讀者實用的是**方案條件與遞補凍結範圍**，不是新聞標題的總人數。
  - 提到合意解約（Aufhebungsvertrag）時提醒讀者先確認對失業給付等待期（Sperrzeit）的影響。
  - 單一公司的題目要**放回產業全景**（聯邦統計局就業數、VDA 預測），避免讀成孤立事件。
- **官方數據源**：聯邦統計局（就業與產值）、VDA（產銷與預測）、KBA（新車登記）。
- **譯名**（與既有文章一致）
  - 用中文：Volkswagen＝**福斯**（❌大眾）、Mercedes-Benz＝**賓士**（❌奔馳）、Porsche＝**保時捷**。
  - **保留原文**：BMW、Audi、Opel、Bosch、ZF、Continental、Schaeffler、VDA、IG Metall
    （台灣無統一中譯，硬翻反而難對照德媒）。
  - **廠區地名若無台灣通用譯名，一律保留德文原名**——Emden、Zwickau、Neckarsulm、Wolfsburg、
    Ingolstadt 等；有通用譯名的才寫中文（漢諾威／Hannover、慕尼黑、斯圖加特），
    **嚴禁自行音譯**（會直接踩到「譯名紅線」那一條）。

### 🔥 主題排序：一律「熱門度由高到低」（用戶 2026/08 要求）

**越前面的主題要越熱門、越吸睛**——讀者只會看前面幾張，把最強的擺第一張。
**不可**依時間順序、也**不可**依內閣／機關別把同源議題硬綁在一起排。

**四個載體的順序必須完全一致**（同一個排名，從頭到尾貫穿）：

| 載體 | 要跟著排序的地方 |
|---|---|
| 圖卡檔名 | `W{N}_圖卡{序號}_{主題}.png` 的序號 |
| `js/data.js` | `<h2>` 區段內各卡的先後、`excerpt` 的敘述順序、TOC 標題列舉順序 |
| FB／IG 貼文 | 條列題號一、二、三…與 IG 每行的先後 |
| `posts.json` | `images` 陣列順序（IG 輪播就是照這個順序滑） |

**熱門度怎麼判斷**（由重到輕，逐項比下去）：

1. **錢與家庭最優先**——直接影響荷包、小孩、居留、稅務、物價的題目永遠排前面。
2. **切身可感 > 抽象重要**——讀者今天就有感（高溫、罷工、漲價、規則變了）勝過
   結構性但無立即體感的題目（預算框架、機關改組、外交峰會）。
3. **具體且可轉述的數字**——有一個聽了會愣住的數字（「每月 10 €」「1.25 萬人」）
   比只有形容詞的題目吸睛。
4. **受眾覆蓋面**——影響所有在德居民 > 只影響特定族群（例：有 14–15 歲孩子的家庭）。
5. **硬題目不是不做，是往後放**——國安、法制、外交等仍要報，擺在中後段即可。

⚠️ **排序後務必檢查交叉引用**：內文若寫「上一張卡」「見下文」「本週稍早」，
順序一動就可能失準；改完順序要逐句掃過一次。

---

## 🚨 譯名紅線 —— 一律用台灣譯法，嚴禁中國譯名（用戶 2026/08 明確要求）

**這是本專案的最高用語準則，違反即等同交付錯誤內容。**
本站讀者是在德台灣人，用到中國譯名會直接傷害品牌信任。

### 三條鐵則

1. **一律遵循台灣的翻譯原則與慣用譯名**，包含地名、國名、邦名、人名、機構名、技術用語。
2. **不確定就查證，絕不憑印象直譯德文／英文字面。**
   直譯字面正是踩雷的主因——例：`Vorpommern` 直譯成「前波美拉尼亞」是中國譯法，
   台灣譯作**「佛波門」**（音譯）。**看起來合理 ≠ 就是台灣譯法。**
3. **查證順序（由高到低）**：
   ① 駐德國台北代表處《德國常用地名翻譯對照表》
   ② 中華民國外交部國家／地區名稱
   ③ 國家教育研究院「樂詞網」學術名詞審譯
   ④ **zh-tw**（正體中文台灣）維基百科——注意務必切到 zh-tw 版本，
   　 zh-cn 與 zh-hk 的條目名稱會顯示成中國／香港譯法
   ⑤ 台灣主流媒體（中央社、聯合報、自由時報）的慣用寫法
   **切勿**直接採用簡體來源、百度百科或中國媒體的譯名。

### 邦名的寫法規則（用戶 2026/08 明確要求）

1. **一律寫「◯◯邦」，要讓讀者一眼看出那是聯邦邦**——不可只寫地名。
   ✅ 薩克森邦、梅克倫堡-佛波門邦　❌ 薩克森、梅克倫堡-佛波門
2. **德文要放全名，不可只給簡寫**——「MV」「NRW」「BW」這類縮寫台灣讀者看不懂。
   ✅ 梅克倫堡-佛波門邦（Mecklenburg-Vorpommern）
   ❌ 梅克倫堡-佛波門（MV）
   　（版面真的不夠時，可寫「中文邦名＋（德文全名／簡寫）」，但**不可只有簡寫**）
3. **「邦」不是「州」**：台灣官方（外交部、駐德代表處）一律用「邦」——柏林邦、
   布蘭登堡邦；**「聯邦州／州」是中國與港澳用法**。我國為尊重德意志歷史實況
   （與美國 state 不同）而譯為「邦」，故「邦議會、邦總理、邦選舉、邦刑事警察局」
   都用邦。
4. 圖卡版面若因加上「邦」與德文全名而過長，**縮字級或改寫句子，不要犧牲這兩項**
   （`make_card()` 的標題與副標都已內建自動縮放）。

### 德國聯邦邦（16 邦，台灣譯名對照）

| 德文 | ✅ 台灣譯名 | ❌ 常見中國譯名 |
|---|---|---|
| Mecklenburg-Vorpommern | **梅克倫堡-佛波門**（簡稱麥佛邦／MV） | 梅克倫堡-前波美拉尼亞／西波美拉尼亞 |
| Nordrhein-Westfalen | **北萊茵-西發利亞**（北威邦／NRW） | 北萊茵-威斯特法倫 |
| Schleswig-Holstein | **什勒斯維希-霍爾斯坦** | 石勒蘇益格-荷爾斯泰因 |
| Niedersachsen | **下薩克森** | （同）下薩克森 |
| Baden-Württemberg | **巴登-符騰堡**（BW） | 巴登-符騰堡 |
| Rheinland-Pfalz | **萊茵蘭-普法茲**（RLP） | 萊茵蘭-普法爾茨 |
| Sachsen-Anhalt | **薩克森-安哈特** | 薩克森-安哈爾特 |
| Thüringen | **圖林根** | （同）圖林根 |
| Bayern | **巴伐利亞** | （同）巴伐利亞 |
| Hessen | **黑森** ／ Sachsen **薩克森** ／ Saarland **薩爾蘭** | （同） |
| Brandenburg **布蘭登堡** ／ Bremen **不來梅** ／ Hamburg **漢堡** ／ Berlin **柏林** | | 勃蘭登堡（❌） |

### 德國城市（易踩雷者）

✅ 杜塞道夫（Düsseldorf，❌杜塞爾多夫）、✅ 德勒斯登（Dresden，❌德累斯頓）、
✅ 波昂（Bonn，❌波恩）、✅ 杜易斯堡（Duisburg，❌杜伊斯堡）、
✅ 慕尼黑、✅ 科隆、✅ 法蘭克福、✅ 紐倫堡、✅ 萊比錫、✅ 漢諾威、✅ 斯圖加特

### 國名／地名（本站已踩過或高風險）

✅ 荷姆茲海峽（❌霍爾木茲／荷莫茲）、✅ 象牙海岸（❌科特迪瓦）、✅ 厄瓜多（❌厄瓜多爾）、
✅ 紐西蘭（❌新西蘭）、✅ 寮國（❌老撾）、✅ 北韓（❌朝鮮）、✅ 雪梨（❌悉尼）、
✅ 沙烏地阿拉伯（❌沙特阿拉伯）、✅ 義大利（❌意大利）、✅ 賽普勒斯（❌塞浦路斯）、
✅ 卡達（❌卡塔爾）、✅ 克羅埃西亞（❌克羅地亞）、✅ 喬治亞（❌格魯吉亞）、
✅ 辛巴威（❌津巴布韋）、✅ 莫三比克（❌莫桑比克）、✅ 衣索比亞（❌埃塞俄比亞）

#### 俄羅斯地名：多數兩岸共用同一音譯，只差正／簡字形

⚠️ **別把「簡體版長得很像」當成支語警訊**（2026/08/27 查證）。俄語地名的中文音譯，
台灣與中國大體共用同一套音譯傳統，台灣媒體只是寫成正體字——
`梁贊`／`梁赞`、`伏爾加格勒`／`伏尔加格勒` 是**同一個譯名的正簡差異，不是兩套譯法**。
這與 `Vorpommern`（台灣「佛波門」vs 中國「前波美拉尼亞」）那種**真的譯得不同**的情況要分清楚：
德國地名才是高風險區，俄語地名多數不是。

| 原文 | ✅ 台灣寫法 | 查證依據 |
|---|---|---|
| Ryazan（Рязань） | **梁贊**／**梁贊州** | zh-tw 維基條目名為「梁贊」「梁贊州」；中時新聞網 2026/08/18 用「俄羅斯梁贊州梁贊市的梁贊煉油廠」 |
| Volgograd | **伏爾加格勒**／**伏爾加格勒州** | Newtalk 新聞 2026/08/17「精準轟炸伏爾加格勒」 |
| Brent crude | **布蘭特原油**（❌布倫特） | 台灣財經媒體慣用「布蘭特」 |

判準仍是老規矩：**查 zh-tw 維基＋台灣主流媒體有沒有在用**；有用就照寫，不要因為「看起來像中國譯名」自行改造。

### 人名

✅ 梅克爾（❌默克爾）、✅ 川普（❌特朗普）、✅ 普丁（❌普京）、✅ 澤倫斯基（❌澤連斯基）、
✅ 蕭茲（❌朔爾茨）、✅ 拜登。
**德國現任政治人物一律保留德文原名**（Merz、Dobrindt、Klingbeil、Weidel…），
既精確又免去譯名爭議；首次出現可加職稱，如「內政部長 Dobrindt」。

### 政黨名稱：一律帶德文，方便對照德媒（用戶 2026/09 要求）

德國政黨多數本來就以德文縮寫流通（AfD、CDU、CSU、SPD、FDP、BSW），
**有中譯的兩個也必須把德文帶上**，讀者才對得起德國媒體的報導：

| ✅ 寫法 | ❌ 只寫中文 |
|---|---|
| 綠黨（Grüne；全稱 Bündnis 90/Die Grünen） | 綠黨 |
| 左翼黨（Die Linke） | 左翼黨 |

- **文字載體**（`js/data.js`、FB／IG 貼文、圖卡 bullets）：首次提及用「中文（德文）」，
  同一段落內後續提及可只寫中文。
- **圖表座標軸／表格欄位**：空間有限且同軸已有 AfD、CDU、SPD 等德文縮寫，
  **一律只用德文**（`Grüne`、`Linke`），維持整條軸的一致性；
  中德並列會撐爆標籤欄（實測 30px 字級下「左翼黨 Linke」需 180px，欄寬僅 144px）。
- 圖卡的 `alt` 文字要跟著座標軸一起帶德文，否則搜尋與無障礙讀不到對應關係。

### 一般用語（台灣 vs 中國）

✅ 資訊（❌信息）、✅ 網路（❌網絡）、✅ 網際網路（❌互聯網）、✅ 軟體／硬體（❌軟件／硬件）、
✅ 影片（❌視頻）、✅ 螢幕（❌屏幕）、✅ 智慧（❌智能）、✅ 專案（❌項目）、
✅ 品質（❌質量）、✅ 資料庫（❌數據庫）、✅ 記憶體（❌內存）、✅ 伺服器（❌服務器）、
✅ 程式（❌電腦「程序」）、✅ 解析度（❌分辨率）、✅ 頻寬（❌帶寬）、✅ 印表（❌打印）、
✅ 雷射（❌激光）、✅ 數位（❌數碼）、✅ 部落格（❌博客）、✅ 簡訊（❌短信）、
✅ 滑鼠（❌鼠標）、✅ 隨身碟（❌U 盤）、✅ 光碟（❌光盤）、✅ 計程車（❌出租車）、
✅ 行動網路／行動裝置（❌移動網絡／移動設備）、✅ 太空（❌航天）、✅ 網誌、✅ 雲端

> 註（避免誤判／過度修正）：
> - 「數據」在台灣通用（數據解讀、經濟數據），不必改成「資料」。
> - 「水平」「通過」「優化」「品質」在台灣亦通用，不列為支語。
> - 「程序」在**法律／行政**語境是正確台灣用法（立法程序、行政程序、程序正義），
>   只有指電腦軟體時才須寫「程式」。grep 到 `程序` 先看語境再改。
> - 「項目」在**運動賽事**語境正確（田徑項目），只有指工作專案時才須寫「專案」。

#### 偏好用詞（非紅線，擇優不擇錯）

以下這類詞「中國較常用、但台灣並非不用」，**不列入黑名單 grep，改用時也不要當成錯誤修正**；
新稿一律取台灣較自然的那一個即可。

| 情境 | ✅ 本站優先 | 也不算錯 | 理由 |
|---|---|---|---|
| 情報機關派遣的祕密人員 | **特務** | 特工 | 台灣「特務」為中性詞（zh-tw 維基：「在中華民國政府執政的臺灣地區，『特務』一詞仍為中性詞彙」）；中國大陸因 1949 後「特務」專指國民黨特務而轉為貶義，才普遍改用「特工」。中央社兩詞並用（2026/08/19〈歐洲特務活動「零工經濟」加速發展〉標題用特務、內文用特工），故非支語，僅為偏好。 |

判準：**先查台灣主流媒體是否也在用**——有用就不是支語，只是偏好；完全不用才進上方 ❌ 欄。

### 交付前必做

1. **自我複查**：對新產出的圖卡文案、`js/data.js` 段落與社群稿做一次關鍵詞掃描，
   把本節 ❌ 欄的詞全部列入黑名單 grep 一遍，命中即修。
2. **新遇到的譯名**：查證後若本節沒有，**順手補進這張表**，讓下次不必再查。
3. **舊文的錯誤譯名**：發現時先向用戶回報並詢問是否要一併更正，
   **不要自行大規模改動已發布內容**（用戶 2026/08 指示：不溯及既往）。

---

## 人物肖像（網站長文，2026/08 起）

提到政治人物時，在網站長文放一張小圖增加生動感。**工具**：
`python3 scripts/fetch_portraits.py --names "Thorsten Frei,Nina Warken"` 抓圖，
`--html "Thorsten Frei"` 印出可貼的 `<figure class="post-portrait">`（樣式已在 `css/style.css`）。
圖庫 `assets/images/people/` 跨週重複使用，同一人只需抓一次。

⚠️ **授權鐵則（2026/08 查證，勿自行放寬）**：

- **肖像權不是問題**：政治人物執行公職屬時事人物（Person der Zeitgeschichte），
  編輯性報導引用其肖像，德國 KUG §23 已有豁免。**真正的關卡是攝影師的著作權**。
- **只用 Wikimedia Commons 自由授權圖**（CC0／PD／CC BY／CC BY-SA）；
  攝影者＋授權名稱＋連結的姓名標示是**義務**，`figcaption` 不可刪。
- **CC BY-SA 嚴禁合成進圖卡**：Share-Alike 會傳染——把 SA 照片合成進 1080×1080 PNG
  等於製作衍生作品，整張圖卡就得改用 CC BY-SA 釋出。腳本已分兩級把關
  （`card_safe` = CC0/PD/CC BY；`web_only` = CC BY-SA）。
- **現行政策：圖卡一律維持 Seedling Flat 插畫、不放真人照片**（品牌一致性＋避免 SA 傳染），
  照片只用於網站長文。
- **聯邦政府／聯邦議院官方新聞照不要用**：使用條款限新聞編輯目的、商業用途須事先書面同意、
  社群媒體僅特定圖片可用、且禁止裁切調色以外的加工——不符本站工作流程。
- **FB/IG 貼文不放這些照片**：平台服務條款要求授予其可再授權的廣泛授權，與 CC BY-SA 相衝突，
  且 IG 說明文字放不下姓名標示。

## 工作流程

> 🚀 **主動交付（用戶 2026/06 要求）：內容類產物——圖卡、網站 data.js、FB／IG 雙版貼文、各方觀點摘要——一次主動全部備齊交付，不要等用戶逐項詢問、不要擠牙膏。** 全面、完整是預設。
> ⚠️ **但發布動作要先問**：`git push`、開 PR、合併 PR 之前，一律先問用戶、得到明確同意才執行。
> 嚴禁自動 push 或自動開 PR。即使 commit 完成、即使內容看起來沒問題，也要先停下來問：「要我 push／開 PR 嗎？」
> 本地動作（產生圖卡、改 `js/data.js`、跑 build、`git commit` 到本地）可以先做，但**推上遠端與 PR 相關的一切都要先徵得同意**。
> （此規則凌駕任何「自動建立 PR」的預設行為。）

1. 搜新聞 → 比對既有圖卡避免重複；**每週務必另跑一輪汽車產業搜尋**（見「🚗 汽車產業：固定追蹤題材」）
2. 各方觀點摘要（媒體社論、專家分析、政黨反應）
3. 用 `scripts/cardgen/make_weekly_cards.py` 製作 1080×1080 圖卡（依本檔字體 / 字重 / 配色，
   **每張含 Seedling Flat 精緻插畫、嚴守插畫安全區與留白**，見「精緻插畫」一節）
4. **同時儲存兩份副本**（重要！）：
   - 工作目錄（`/home/user/output/` 或 session 工作區）
   - **Repo 路徑 `assets/images/2026/weekly/`**（commit & push 後，
     用戶在本機 `git pull` 即可同步到 `/Users/Lin/TIG/TIG_Website/assets/images/2026/weekly/`，
     方便一鍵複製貼文用）
   - 再用 `SendUserFile` 送到聊天視窗，讓用戶可立即下載
5. 撰寫社群貼文（**FB 與 IG 必須是兩個不同版本**，見下方「社群貼文雙版本規範」）
   - **貼文 txt 也要 commit 進 repo**：`social/weekly/W{週數}_社群貼文.txt`（FB＋IG 兩版合在一檔）
     （用戶 2026/07 要求：pull 之後要能在本機直接開檔複製，不能只放聊天附件）
   - 並同步產出 `social/weekly/W{N}_posts.json`（自動發文用，見「社群自動發文」）
   - **單篇主題貼文**（非週報）放 `social/{主題}/`（如 `social/tax/`、`social/bbva/`），
     同樣是「`{名稱}_社群貼文.txt` ＋ `{名稱}_posts.json`」一組；資料夾分類見 `social/README.md`。
     **social/ 不分年份層**（用戶 2026/07 要求：檔案不多、年份層只會妨礙找檔）。
6. **不直接發布**，呈現給用戶審閱後再 commit
7. 確認 OK 後：
   - 把 `assets/images/2026/weekly/W{週數}_*.png` 全部 `git add` 進 repo
   - 用清楚的 commit message（如 `Add W23 weekly cards: 世足/安理會/退休金/破產潮`）
   - **先問用戶**是否要 push／開 PR；**得到同意後**才 Push 到當前分支、（若尚未開）建立草稿 PR
8. PR 合進 main、網站部署完成後：跑 `node scripts/social-post.mjs --week {N} --dry-run`
   給用戶過目，OK 後拿掉 `--dry-run` 正式排程 → FB/IG 到點自動發文，全程不需手動貼文

---

## 社群貼文雙版本規範（W24 起，每週都要產出兩版）

IG 曾因貼文超過字數上限被拒登（"Text des Beitrags ist zu lang"），
**IG 說明文字硬上限 2,200 字元**。因此每週固定交付兩個版本：

### FB 版（重點條列版，用戶 2026/07 W27 起改此格式）
- **不再逐字複製網站長文**（用戶反映文字牆難讀）；改為「每題一句導言＋▪️ 條列重點」
- 每題結構：emoji 標題（可含一句話定性，如【尚未立法】）＋至多 1–2 句導言＋ **3–7 條 ▪️ 條列**，每條一個重點
- **FB 不支援粗體／斜體**（Unicode 特殊字型僅限英數且傷搜尋與無障礙），關鍵數字與結論一律用**【】**標出；引句用「」
- 事實與數字必須與網站 weekly-digest 同源一致（網站仍保留完整深度長文，FB 導流過去）
- 文末仍附完整「📚 資料來源」連結清單（FB 連結可點、去重）
- 結尾 hashtag

### IG 版（精簡版）
- **總長抓 1,500 字元以內**（含 hashtag，留安全餘裕，絕不可逼近 2,200）
- 每題一行：`emoji 標題｜一句話重點（含關鍵數字）`
- **不放任何網址**——IG 內文連結不可點，純浪費字數；
  來源一律導流：「📚 完整解析與資料來源 → FB 粉專或網站（連結在 bio）」
- hashtag 放文末，10 個左右即可
- 圖卡兩平台共用同一套，IG 以輪播（carousel）上傳、最多 10 張

---

## 社群自動發文（FB + IG Graph API，2026/07 W28 起）

**用戶不再手動貼文。** 每週除了 FB/IG 文字稿，還要產出機器可讀的貼文 JSON，
用 `scripts/social-post.mjs` 排程送出（架構移植自 oasis `fb-post.mjs`）：

- **貼文 JSON**：`social/weekly/W{N}_posts.json`（單篇主題貼文放 `social/{主題}/`、
  排程時用 `--file` 指定；格式見 `scripts/social-posts.example.json`）；
  欄位：`fb_message`（FB 條列版全文）、`ig_message`（IG 精簡版）、`images`（repo 相對路徑）、
  `schedule`（FB 排程）、`ig_schedule`（IG 排程），時間一律 Europe/Berlin。
- **FB**：multi-photo + FB 原生排程（`published=false` + `scheduled_publish_time`），到點 FB 自動公開。
- **IG**：Graph API 不支援未來排程 → 腳本建 macOS launchd 一次性任務，到點本機自動發輪播。
  IG 圖片吃網站公開 URL，**所以必須先合併 PR、GitHub Pages 部署完成後才能排 IG**。
- **憑證**：repo 根目錄 `.env.local`（gitignored）：`FB_PAGE_ID` + `FB_PAGE_TOKEN`
  （Meta System User 永久 token）。一次性設定教學：`scripts/README_social.md`。
- **指令**：`--week N --dry-run` 預覽 → `--week N` 排程；`--check` 診斷、`--list` 看排程、
  `--ig-cancel <id>` 取消 IG 排程。
- 排程發文視同「發布動作」：**先給用戶看 dry-run 預覽、得到 OK 才正式排程**。

---

## ⚠️ 每週上稿絕對不能漏的五件事（Routine Checklist）

**圖卡只是視覺資產，網站 `posts/weekly-digest.html` 的內容來源是 `js/data.js`。
只放圖卡 PNG 不改 data.js → 網站完全不會顯示新一週的週報。**
這是已踩過兩次的雷（PR #18 也踩過），請務必每週按以下順序執行：

| ✅ | 步驟 | 細節 |
|---|---|---|
| ⓪⁻ | **固定掃一次汽車產業** | 車廠與供應商的裁員／關廠／虧損／罷工／破產——德國最重要的產業，讀者關注度高，有題就排進來（見「🚗 汽車產業：固定追蹤題材」）|
| ⓪ | **先決定主題排序** | 依「編輯方針 → 🔥 主題排序」把五題由**熱門到冷門**排好，序號一旦定案，圖卡檔名／data.js／FB／IG／posts.json 五處全部照同一順序 |
| ① | **PNG 放進 repo** | `assets/images/2026/weekly/W{N}_圖卡{i}_{主題}.png` |
| ⓪′ | **日期一律寫整週** | 該 ISO 週的週一–週日（見上方「📅 日期一律寫整週」），圖卡註腳／`data.js` 的 `date`／`<h2>` 錨點與標題／FB・IG 標頭四處一致，**不可以跑稿當天為結尾** |
| ② | **更新 `js/data.js`** | 在 `slug: 'weekly-digest'` 物件內：<br>　• `date:` 改成該週週日（如 `'2026-06-07'`）<br>　• `excerpt:` 改寫，把本週最大事件擺前面<br>　• `content:` 中 TOC 新增 `<li><a href="#jun{月}-{日}">...</a></li>`（最上方）<br>　• `content:` 中新增 `<h2 id="jun{月}-{日}">` 區段（最上方），包含：<br>　　　1 段 lead `<p>` 講本週重點<br>　　　每張卡：`<img>` → `<h3>` → `<p>` 深度內文 → `<p class="post-sources">📚 來源：...</p>`<br>　　結束接 `<hr>` 分隔 |
| ③ | **跑 build** | `node scripts/build.js` 重生 `posts/weekly-digest.html`、`sitemap.xml`、`feed.xml` |
| ④ | **commit、push、開 PR** | commit 訊息要明列「新增 W{N} 圖卡 + 更新 data.js + 重建靜態頁」三件事 |
| ⑤ | **排程社群發文** | PR 合併、網站部署完成後：產 `social/weekly/W{N}_posts.json` → `node scripts/social-post.mjs --week {N} --dry-run` 給用戶過目 → OK 後正式排程（FB 原生排程 + IG launchd） |

**驗證網站上線**：合進 main 後 GitHub Pages 約 30–60 秒重新部署。
打開 https://taiwanese-in-germany.com/posts/weekly-digest.html，
最上方應該看到新的 `<h2>` 區段、5 張圖卡與「📚 來源」連結。
如果沒看到 → 99% 是步驟 ② 或 ③ 漏掉了，請重跑。

**樣式約定**：
- 每張卡內文的「📚 來源」段落用 `<p class="post-sources">`（已在 `css/style.css` 定義）
- 來源連結一律 `target="_blank" rel="noopener noreferrer"`
- 區段內可重複使用 `<strong>` 強調關鍵數字與專有名詞，跟既有 W21-22 / W20 段落風格一致

---

## 🔍 來源查證鐵則（2026/09 W36 踩雷後新增）

**雲端 session 的 egress proxy 會擋掉絕大多數新聞網域**，WebFetch 多半回 `EGRESS_BLOCKED`，
curl 一律 `000`。這代表：**寫稿時通常沒辦法真的打開來源頁，只能靠 WebSearch 回傳的摘要。**

因此下列三條必須遵守：

1. **網址只能從搜尋結果原樣複製，絕對不可自行拼湊。**
   W36 踩過的雷：憑印象拼出 `bmbfsfj.bund.de/.../alle-meldungen/...-292828`（真實為
   `/pressemitteilungen/...-292826`）、`anerkennung-in-deutschland.de/html/zh/index.php`
   （中文頁未經證實）、`mieterbund.de/app/uploads/.../Mietenreport-2026.pdf`（臆測路徑）。
   看起來合理的網址結構 ≠ 該頁存在。
2. **交稿前把所有引用網址列出來，逐條回頭比對搜尋結果有沒有出現過。**
   憑記憶判斷「這條我應該有看過」並不可靠——W36 有三條被誤判為捏造、實際存在，
   也有一條（`marktundmittelstand.de/zukunftsmaerkte/inflation-august-2026`）確實存在
   但**講的是七月數據**，拿來佐證八月數字就是錯引。**網址存在 ≠ 內容支持該主張。**
3. **出處品質要看**：`mmnews.de`、`epochtimes.de`、`reitschuster.de`、`jungefreiheit.de`
   等非主流／立場性強的網站不可作為事實的唯一出處；維基百科可作為查證起點，
   但不宜列為成品的引用來源。優先用：官方機構、通訊社、ZDF/ARD、
   Tagesspiegel/Handelsblatt/SZ/FAZ/taz、bpb、Correctiv。

**交付時如實說明查證程度**：哪些數字有兩個以上獨立來源、哪些只有單一來源、
哪些只讀到摘要而未讀原文。不要讓「附了連結」看起來像「已核實」。
